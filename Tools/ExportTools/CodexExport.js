 ...
 * converts a table into Json compatible with JaDa_Encyclopedia
...
function CollectRows(array, index, colIdx) {
    ...
    var rowIdx = index - 1;
    var list = new Array();
    while (true) {  //there should be an empty row between PageName or this will not work !
        rowIdx += 1;
        if (array[rowIdx][colIdx] != "") {
	var subData = {"PageName":JSON.stringify(array[rowIdx][colIdx]),
			"Image": JSON.stringify(array[rowIdx][Headers["Image"]]),
			"Description": JSON.stringify(array[rowIdx][Headers["Description"]])};
            list.push(JSON.stringify(subData));
        } else {
            break;
        }
    }
...
function NextEntry(sheet) {
    ...
    while (index < maxIndex) {
        ...
        while (index <= maxIndex) {
            ...
            if (array[index][Headers["EntryId"]] != "") {
                entryId = array[index][Headers["EntryId"]];
                break;
            }
        }
	...
	aEntry["EntryId"] = String(array[index][Headers["EntryId"]]);
        aEntry["Title"] = array[index][Headers["Title"]];
        aEntry["Type"] = array[index][Headers["Type"]];
        aEntry["Pages"] = JSON.stringify(CollectRows(array, index, Headers["PageName"]));

        allEntry.push(JSON.stringify(aEntry)); 
    }
    return (allEntry);
}
