...
ICF.ETroopEx.Version = 101; // 1.01

//=============================================================================
 /*:
 * @plugindesc v1.01c This plugin allows more conditions to battle events,
 * add encounter conditions and to concatenate troops.
 ...
 * 
 *  - Add specified game, map or party switch to conditions.
 *    You can also add game switchs that must be off.
 *    You can place all same-type switchs you need in same line.
 * 
 ...
 *  - Add a variable condition. There are six types: min value, higher than,
 *    lower than, max value, equal and not equal.
 *    Optionaly you can add an opt value to use map and party
 *    variables this way:
 * 
 * encountermapvariable x value
 * encounterpartybattlevariable x value [min/max/avg]
 * 
 *  - Party variables can have an extra parameter. If omited
 *    it will check sum of members specified variables.
 *    -min: check minimun of members specified variables.
 *    -max: check maximun of members specified variables.
 *    -avg: check average value of members specified variables.
...
//=============================================================================
 /*:es
 * @plugindesc v1.01c Este complemento permite más condiciones en los eventos
 * de batalla, condiciones para encuentros y concatenar tropas.
 * @author ICF-Soft [http://icfsoft.blogspot.com.es/]
 *
 * @param Developer HaltJS
 ...
 * [prefix]variabledif x value
 * 
 *  - Añade condición de variable. Hay seis tipos: mínimo, mayor que,
 *    menor que, máximo, igual y diferente.
 ...
 * 
 * Para añadir condiciones para que un encuentro enemigo pueda aparecer
 * se utilizan los siguientes comentarios:
 * 
 * encounterswitch x x x
 * encounteroffswitch x x
 * encountermapswitch x x x x
 * encounterpartyswitch x x x
 * 
 *  - Añade interruptores del juego, mapa o del grupo.
 *    Tambien interruptores del juego que deban estar desactivados.
 *    Puedes poner todos los necesarios en la misma linea.
 * 
 * encounter[opt]variable x value
 * encounter[opt]variablehigh x value
 ...
 *  - Añade condición de variable. Hay seis tipos: mínimo, mayor que,
 *    menor que, máximo, igual y diferente.
 *    Opcionalmente puedes añadir un valor en opt como map (de mapa),
 *    o party (del grupo):
 * 
 * encountermapvariable x value
 * encounterpartybattlevariable x value [min/max/avg]
 * 
 *  - Los de grupo pueden llevar un parámetro extra. Si se omite se usará
 *    la suma de dicha variable por cada miembro.
 *    -min: el mínimo del grupo.
 *    -max: el máximo del grupo.
 *    -avg: la media del grupo.
...
DataManager.isDatabaseLoaded = function() {
	...
	ICF.NotetagsProcessor.eTroopEx($dataTroops);
	ICF.ETroopEx.Procesed = true;
    }
    return true;
};

ICF.NotetagsProcessor.eTroopEx = function(group) {
...
ICF.ETroopEx.Checking = false;

ICF.ETroopEx.CustomSwitch = function(mapid, evid, switchname, value) {
	var _key = [mapid, evid, switchname];
	var _value = (value.toLowerCase() === "true");
	$gameSelfSwitches.setValue(_key, _value);
}

ICF.ETroopEx.ProcessComment = function(page, obj, content) {
	var args = content.split(" ");
	if (ICF.TroopEx.Checking == 1) {
		page._customreq = page._customreq + content + '\n';
	} else if (ICF.TroopEx.Checking == 2) {
		obj.customreq = obj.customreq + content + '\n';
		...
		} else if (args[0].toLowerCase() == "encounterswitch") {
			args.splice(0, 1);
			obj.switchs = obj.switchs.concat(args);
		} else if (args[0].toLowerCase() == "encounteroffswitch") {
			args.splice(0, 1);
			obj.offswitchs = obj.offswitchs.concat(args);
		} else if (args[0].toLowerCase() == "encountermapswitch") {
			args.splice(0, 1);
			obj.mapswitchs = obj.mapswitchs.concat(args);
		} else if (args[0].toLowerCase() == "encounterpartyswitch") {
			args.splice(0, 1);
			obj.partyswitchs = obj.partyswitchs.concat(args);
		} else if (args[0].toLowerCase().match(/(encounter)((?:map)|(?:party))?(variable)((?:high)|(?:less)|(?:min)|(?:max)|(?:equal)|(?:dif))?/i)&&(RegExp.$1.length + RegExp.$2.length + RegExp.$3.length + RegExp.$4.length == args[0].length)) {
			var code = (RegExp.$4 == "high")? 2 : (RegExp.$4 == "less")? 4 : (RegExp.$4 == "max")? 5 : (RegExp.$4 == "equal")? 0 : (RegExp.$4 == "dif")? 1 : 3;
			if (RegExp.$1 == "battle") page._battlevars.push([code, args[1], args[2]]);
			else if (RegExp.$2 == "map") obj.mapvars.push([code, args[1], args[2]]);
			else if (RegExp.$2 == "party") obj.partypvars.push([code, args[1], args[2], args[3]]);
			else obj.vars.push([code, args[1], args[2]]);
		} else if (args[0].toLowerCase() == "customreq:") {
			page._customreq = page._customreq + content.substring(10) + '\n';
			ICF.Events.Checking = 1;
		} else if (args[0].toLowerCase() == "encountercustomreq:") {
			obj.customreq = obj.customreq + content.substring(19) + '\n';
			ICF.Events.Checking = 2;
...
ICF.ETroopEx.CheckVar = function(code, value1, value2) {
	switch (code) {
		case 0:
...
ICF.ETroopEx.EncounterConditions = Game_Player.prototype.meetsEncounterConditions;
Game_Player.prototype.meetsEncounterConditions = function(encounter) {
	if (!ICF.TroopEx.EncounterConditions.call(this, encounter)) return false;
	var obj = $dataTroops[encounter.troopId];

	for (i = 0; i < obj.switchs.length; i += 1) {
		if (!$gameSwitches.value(obj.switchs[i])) {
			return false;
		}
	}
	for (i = 0; i < obj.offswitchs.length; i += 1) {
		if ($gameSwitches.value(obj.offswitchs[i])) {
			return false;
		}
	}
	for (i = 0; i < obj.mapswitchs.length; i += 1) {
		var key = [this._mapId, 0, obj.mapswitchs[i]];
		if ($gameSelfSwitches.value(key) !== true) {
			return false;
		}
	}
	for (i = 0; i < obj.partyswitchs.length; i += 1) {
		if ($gameParty.selfswitch(obj.partyswitchs[i]) !== true) {
			return false;
		}
	}
	for (i = 0; i < obj.vars.length; i += 1) {
		if (!ICF.TroopEx.CheckVar(obj.vars[i][0], $gameVariables.value(obj.vars[i][1]), obj.vars[i][2])) {
			return false;
		}
	}
	for (i = 0; i < obj.mapvars.length; i += 1) {
		var key = [this._mapId, 0, obj.mapvars[i][1]];
		if (!ICF.TroopEx.CheckVar(obj.mapvars[i][0], $gameSelfVariables.value(key), obj.mapvars[i][2])) {
			return false;
		}
	}
	for (i = 0; i < obj.partypvars.length; i += 1) {
		var code = -1;
		if (!!obj.partypvars[i][3]) {code = ["min", "max", "avg"].indexOf(obj.partypvars[i][3].toLowerCase());}
		if (code == 0) {if (!ICF.TroopEx.CheckVar(obj.partypvars[i][0], $gameParty.minselfvariable(obj.partypvars[i][1]), obj.partypvars[i][2])) {
			return false;}
		} else if (code == 1) {if (!ICF.TroopEx.CheckVar(obj.partypvars[i][0], $gameParty.maxselfvariable(obj.partypvars[i][1]), obj.partypvars[i][2])) {
			return false;}
		} else if (code == 2) {if (!ICF.TroopEx.CheckVar(obj.partypvars[i][0], $gameParty.avgselfvariable(obj.partypvars[i][1]), obj.partypvars[i][2])) {
			return false;}
		} else {if (!ICF.TroopEx.CheckVar(obj.partypvars[i][0], $gameParty.selfvariable(obj.partypvars[i][1]), obj.partypvars[i][2])) {
			return false;}
		}
	}
